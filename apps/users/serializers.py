...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        user.set_password(validated_data["password"]) # 把明文变成密文
        user.save()
        return user
    ...
    # validate 作用于所有字段，因为我们想直接将用户名赋值给手机号，这也是在 models 中设计 mobile 可以为空的原因
    def validate(self, attrs):
        attrs["mobile"] = attrs["username"]
        del attrs["code"] # user 表中不需要存 code，所以删除
        return attrs
